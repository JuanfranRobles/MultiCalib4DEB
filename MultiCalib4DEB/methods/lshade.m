...
function [q, info, solution_set, bsf_fval] = lshade(func, par, data, auxData, weights, filternm)
   ...
   global lossfunction max_fun_evals max_calibration_time num_runs refine_initial  
   ...
   fprintf('Using L-SHADE multimodal algorithm. \n');
   ...
   for run = 1:(min(num_runs, length(random_seeds)))
      ...
      while nfes < max_nfes && (toc(run_time_start)/60) < max_calibration_time
         ...
         if max_calibration_time > 0
            ...
            if verbose
               fprintf('Time accomplished: %d of %d minutes (%d %%) for the %d run \n', ...,
